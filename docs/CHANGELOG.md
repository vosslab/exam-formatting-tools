...
- Replaced the obsolete `docs/ROADMAP.md` content. The previous file described the long-removed ODT pipeline (Phases 1-4 covering `odt_utils.py`, `extract_odt_styles.py`, etc.), which has been entirely superseded by the DOCX pipeline. The new ROADMAP names the current state, near-term tuning work (image-choice column caps measured via `tools/measure_image_choices.py`), and remaining known doc gaps. Out-of-scope section explicitly bans reintroducing ODT output and the one-shot HTML-to-DOCX path that was deleted on 2026-05-07. Confirmed both `LICENSE.LGPL_v3` (source code) and `LICENSE.CC_BY_4_0` (non-code material) are present at the repo root, so the previously-flagged license gap was incorrect.
