...
- `ef_tools/bbq_html.pull_tables` flattens a single-row, multi-cell, short-token table (bptools sequence strips such as `5'-ANNN-3'` drawn one monospace cell per token) to inline text instead of rasterizing it (`single_row_table_text`, `SEQUENCE_TOKEN_MAX_CHARS = 8`); non-breaking spaces become plain spaces. `palindrome_sequence_match` now prints as text prompts. Multi-row tables (gels, pedigrees) still render to PNG.
