...
- Added `ef_tools.docx_builder.add_first_page_heading`, which puts the title flush left and the name/score block at a single tab stop on the same line.
...
- The first-page heading is two lines instead of three: the title sits flush left with the name rule beside it and the score line beneath, using the full page width. `heading_1_centered` is now false, `boilerplate_tab` (1.4 in) is replaced by `heading_block_tab` (4.0 in), and `heading_space_before`/`heading_space_after` drop to 0.0/0.04 in.
...
- Added `bbq_tasks_to_exam_yaml.py`: builds exam YAML from a biology-problems-website task CSV (`subject,topic,script,flags,input,notes`, aliases from the website `bbq_settings.yml`) with **one question per resolved task**. Each generator runs with `-d 1 -x 1` in a temporary directory; a candidate that parses to a skipped bbq type or fails the mode rule triggers a fresh run, up to `MAX_ATTEMPTS = 3`, after which the task is reported as `SKIPPED <label>: <reason>` on stderr and left out (a generator crash still raises). `-q/--quiz` (default) accepts any MC/MA/MAT/ORD; `-e/--exam` keeps only questions that `ef_tools.zip_grade.classify_question` rates OK (2-5 lettered options). `-t/--title` defaults to `Quiz` or `Exam` by mode. The CSV `topic` column becomes a `chapter` heading whenever it changes. Outputs: `<stem>.yml`, `<stem>-key.txt`, `<stem>_files/*.png`.
- Added `ef_tools/bbq_tasks.py` mirroring the website `run_bbq_tasks.py` CSV contract: `{key}` path aliases expanded over three passes then `~`, script aliases as `NAME`/`@NAME` with str or list values (`YMATCH` = two scripts = two tasks), `flags` alias-expanded and `shlex.split`, `input` appended as `-y <path>` (bare basenames join the script directory for the five yaml-driven generators), relative paths anchored at `bp_root`, subprocess `PYTHONPATH` = bp repo root (parent of `problems/`) plus the current path. `generate_question(task, pythonpath, reject_fn, candidate_fn=...)` exposes the candidate function as a test seam.
- Added `ef_tools/bbq_html.py`: `split_question_code` strips the leading `<p>CRC</p>`; `pull_tables` removes every `<table>` and returns their HTML for rasterization; `clean_inline_html` walks the lxml tree and maps `<h1>`-`<h6>` to bold lines, `<p>`/`<div>`/`<br>`/`<hr>`/`<ul>`/`<ol>` to newlines, `<li>` to `- `, drops `<span>`/`<font>` wrappers and HTML comments, keeps sub/sup/b/strong/i/em, and raises on any other tag; `write_table_pngs` rasterizes tables with `TableRenderer.render_table_png` into `<stem>_files/` and returns YAML-relative paths.
- Added `ef_tools/bbq_parse.py`: `parse_bbq_line` returns `{'question', 'answer': {'code', 'type', 'letters'}, 'statement_tables', 'choice_tables'}`; MC/MA letters index the `Correct` markers; MAT shuffles `choices_list` and records the letter per prompt; ORD becomes a matching-style block with `prompts_list = ['Position 1', ...]` and shuffled items; NUM/FIB/FIB_PLUS return None (`SKIPPED_TYPES`); `attach_table_images` fills `images`/choice `image` after rendering; `format_answer_key` numbers blocks with `question_span` (`1. B   c555_9c1d   chargaff.py`, `Q5-8. 5=C 6=A 7=D 8=B   ce62_1dbe   yaml_match_to_bbq.py (genetic_disorders)`).
- Added `ef_tools/exam_yaml_writer.py` (`write_exam_yaml`) holding the apostrophe-aware double-quote representer that `html_to_exam_yaml.py` previously defined inline; all three YAML writers use it.
- Added `ef_tools.question_utils.question_span(question)` as the single owner of "rows per question block" (`max(1, len(prompts_list))`); `count_total_questions`, `ef_tools/zip_grade.py`, `yaml_to_exam_docx.py`, and the answer key all call it.
- Added `tests/e2e/e2e_bbq_tasks_quiz.py` (outside pytest per `docs/E2E_TESTS.md`): runs a 4-row CSV (chargaff, YMCS dna_structure, YMATCH genetics_terminology, `who_father_html --easy`) through the new script and `yaml_to_exam_docx.py` in quiz and exam mode, then checks rendered PNGs exist, one key line per block with the highest number equal to the `question_span` sum, DOCX paragraphs free of `<p`/`<span`/`<table`, at least one inline shape, and `validate_zip_grade_yaml.py` passing on the exam YAML. Passed: 9 questions, 0 skipped, both modes.
- Added `tests/test_bbq_html.py`, `tests/test_bbq_tasks.py`, and rewrote `tests/test_bbq_to_exam_yaml.py` as `tests/test_bbq_parse.py` (`git mv`) around the new contracts; added a `question_span` test.

### Behavior or Interface Changes

- `bbq_to_exam_yaml.py` now delegates to `ef_tools.bbq_parse`: MAT emits `prompts_list`/`choices_list` (previously a `table` with `Prompt`/`Match` columns, which printed the answers and contradicted `docs/YAML_EXAM_FORMAT.md`); ORD emits position blanks plus shuffled items (previously `choices` in answer order); `date` is today's date (previously hardcoded `2026-04-02`); NUM/FIB/FIB_PLUS are skipped instead of silently dropped alongside unknown types (unknown types now raise); a `<stem>-key.txt` answer key is written; drawing tables are rendered to `<stem>_files/*.png`. Verified on `site_docs/genetics/topic01/bbq-MATCH-genetic_disorders-questions.txt` (99 blocks, 396 numbered rows, key lines `Q1-4.` ...).
- `yaml_to_exam_docx.build_document(exam_data, output_path, base_dir='.')` resolves relative `image`, `images`, choice `image`, and `choices_list` image paths against `base_dir`; `main()` passes the input YAML's directory, so a YAML and its `<stem>_files/` folder move together. `html_to_exam_yaml.py` writes image paths relative to the output YAML (`relativize_image_paths`) to match.
- `ef_tools.zip_grade._classify_question` is now public `classify_question`; its private `_question_span` copy was removed in favor of `question_utils.question_span`.
- `source_me.sh` prepends `~/nsh/PROBLEMS/qti-package-maker` to `PYTHONPATH` when present (same pattern as `biology-problems/source_me.sh`); `pip_requirements.txt` adds `playwright`; `tests/test_import_requirements.py` whitelists `qti_package_maker` as a sibling-repo import. `docs/INSTALL.md` documents the sibling checkouts and `playwright install chromium`.
- `.gitignore` adds `output_smoke/` and `output_quiz/`.
- The seven CLI scripts moved from the repo root into `launchers/` (`git mv`) to satisfy the new root-script budget gate (`tests/test_root_script_budget.py`: at most six tracked root `.py`/`.sh` files including `source_me.sh`; the repo had eight). Commands are now `python3 launchers/<script>.py`; `source_me.sh` exports the repo root on `PYTHONPATH` so the launchers import `ef_tools`, and `tests/conftest.py` adds `launchers/` to `sys.path` because tests import the CLIs as modules. All docs, README, and the E2E script updated.
- `tools/measure_image_choices.py` moved to `devel/measure_image_choices.py`: the new `tests/test_support_dirs_not_imported.py` gate reserves `tools/` for utilities that do not import repo packages, and the rig imports `ef_tools` and `yaml_to_exam_docx`.
- Matching prompts can carry images: `prompts_list` entries may be `{text, image}` objects (same shape as image choices). `ef_tools.docx_builder.add_matching_prompt` renders `___ N.` followed by the drawing inline; `styles/exam_styles.yaml` adds `prompt_image_max_height` (1.40 in, pedigrees) and `prompt_strip_max_height` / `prompt_strip_min_aspect` (0.40 in for one-row strips with aspect >= 4, so short and long DNA strips share one cell size). `choices_list` with image objects now uses the tabbed image-choice layout like MC image choices. `bbq_parse` records `prompt_tables`, and the new shared `bbq_parse.render_record_tables` rasterizes statement, choice, and prompt tables (both bbq launchers call it instead of duplicating the loop). `palindrome_sequence_match` and the `write_pedigree_match*` generators now print. A first attempt flattened sequence strips to inline text; the user rejected it (the boxed cells carry meaning), so every table renders as an image and non-breaking spaces become plain spaces in text.
- Added a repo-root `bbq_settings.yml` (copy of the website's `paths` and `script_aliases`) and made `-s/--settings` optional on `bbq_tasks_to_exam_yaml.py`, defaulting to it; the website file still works via `-s`. README, USAGE, INSTALL, and the E2E script use the default.
- `tests/conftest.py` adds the `qti-package-maker` sibling checkout to `sys.path` when present, so `pytest tests/` passes without `source source_me.sh`. `qti_package_maker` is declared in `pip_extras.txt` (survives template propagation, unlike the vendored whitelist).

### Fixes and Maintenance

- Synchronized shared style guides, tests, and repository support files from the starter template. Cleared the new gates: `tests/test_function_typing.py` (annotations added to every function and test across `ef_tools/`, `launchers/`, `devel/measure_image_choices.py`, and `tests/`; open-typed callables and lxml elements use `object`), `tests/test_markdown_links.py` (links re-pointed after the `launchers/` and `devel/` moves), `tests/test_root_script_budget.py`, `tests/test_support_dirs_not_imported.py`, `tests/test_source_file_line_limit.py`. `tests/test_docx_choice_styles.py`, `tests/test_test_naming_conventions.py`, and the E2E script import `file_utils` (renamed from `git_file_utils` by propagation).
- Refreshed the documentation set from current repository evidence: the instructor-facing `README.md`, architecture and file-structure maps, install and usage routes, file-format, troubleshooting, FAQ, development, roadmap, related-project, news, and release-history docs. Added two managed README visuals from a generated exam: a full first-page DOCX render and a separately trimmed gel-electrophoresis problem asset. Removed the obsolete composite media contact sheet; the DOCX page render was captured with macOS Quick Look after headless LibreOffice conversion aborted.

### Removals and Deprecations

- Removed `test_combined_yaml_no_paragraph_uses_choice_base_style` from `tests/test_docx_choice_styles.py`: it read the untracked, absent `Final_Exam/Final_Exam_2A_2B_combined.yaml` and failed on every fresh checkout (fragile per `docs/PYTEST_STYLE.md`). The remaining `Choices N` style tests cover the same rule with synthetic data.

- Removed `devel/submit_to_pypi.py` (1104 lines, unreferenced since the initial commit; the template now ships `devel/make_release.py` and this repo has no `pyproject.toml`). It was the only file over the new 999-line source limit.

### Decisions and Failures

- `tests/test_podman_disk_budget.py` (vendored) fails on this machine because no Podman machine is running (`podman system df` cannot connect); it is an environment check, not a repo defect. Start the machine or add a skip upstream.
- The website `genetics_tasks1.csv` was split at the user's request: `genetics_tasks1.csv` = genetic_disorders, dna_structure, dna_profiling (34 rows); mendelian through chi_square moved to a new `genetics_tasks2.csv` (45 rows); the previous `genetics_tasks2.csv` (gene_mapping ... phylogenetics) became `genetics_tasks3.csv` via `git mv`. `all_tasks.py` globs the folder, so no runner references changed.
- One task, one question: the website CSV is reused with no new columns; `count`, candidate pools, and sampling were rejected as batch-generation machinery that does not match building one quiz at a time.
- Regenerate, never edit: a six-choice MC in exam mode is retried, not trimmed to five, so authored questions stay intact. Genetics scripts that exceed five choices do so by fixed flags (`-c 6`, `*-6_choices`), so `MAX_ATTEMPTS = 3` fails fast with a clear reason and the fix is a CSV flag edit.
- NUM, FIB, and FIB_PLUS are skipped by user decision (no print form); HOTSPOT has no bbq text representation and is out of scope; ORDER prints as matching-style position blanks.
- Tables are rendered by importing `qti_package_maker.html_to_image.render_table.TableRenderer` rather than copying it. The plan expected its `is_drawing_table` detector to gate rendering, but a pre-flight parse of every genetics bbq line in `site_docs/` (11439 printable, 792 skipped types) found 361 tables styled only at the `<table>` element (`beadle_tatum` pathway arrows, `unique_cross_*` genotype grids) that the cell-level detector misses, so `pull_tables` rasterizes every table. The same pre-flight surfaced `<h2>`-`<h5>`, `<hr>`, and HTML comments (`complementary_sequences`), all now handled, and `palindrome_sequence_match` whose matching prompts are table drawings: those raise `bbq_parse.UnprintableQuestion`, which `generate_question` treats as a retry reason (the Matching Prompt paragraph is text only). The first full-CSV run also hit `blood_type_agglutination_test`, whose choices are a tray table holding four nested well tables: `pull_tables` now takes outermost tables only (`.//table[not(ancestor::table)]`), and a choice with several sibling tables raises `UnprintableQuestion` at parse time (one image per choice dict) so it becomes a retry/skip instead of a crash. Images render after the full statement text; placing them mid-statement is a possible follow-up.
- The website's `--no-hidden-terms --allow-click` flags no longer exist in bptools (anti-cheat is now opt-in via `--hidden-terms`/`--noclick-div`), so generators run with no anti-cheat flags at all. First run of the CLI failed with `unrecognized arguments: --no-hidden-terms --allow-click` before this was removed.
- bptools keyword styling (`<strong><span style=color>term</span></strong>`) survives as bold text in print; only the color is dropped.
- Fixed task-CSV DOCX output so bptools hex color spans survive the BBQ-to-YAML handoff and become colored Word runs; unrelated span CSS remains excluded from the printable format. Wide image choices such as DNA sequence strips now stack at the printable page width and keep their complete choice block together across page breaks instead of being squeezed into tiny 4/5-column images.
- Improved the genetics quiz layout: plain-text matching blanks pair into two columns, hard-break continuation paragraphs use `Question Follow`, long four- and five-choice text sets stack before wrapping, choice paragraphs receive a small separation from the stem, and only very wide image choices (aspect ratio at least 8.0) use the full-width strip layout.
- Refined the rendered quiz after screenshot review: matching answer choices and numbered blanks now stay chained as one block when they fit on the next page, long image-choice captions stack with their labeled images instead of wrapping into neighboring options, and the configured choice-to-stem spacing is applied consistently.
- Locked rasterized table-image sizing to the renderer's intrinsic geometry: table PNGs use a configurable 96 CSS-pixels-per-inch baseline before the existing role-specific page caps, preventing small source drawings from being enlarged while still allowing large maps and gels to fill their available width.
- Regenerated the managed README screenshots from the current Genetics Quiz 1 DOCX and current gel asset, replacing stale visuals from the earlier quiz build.
- Added the self-contained `ef_tools/docx_table_builder.py` backup path. BBQ conversion now preserves source HTML beside rasterized table images, and `yaml_to_exam_docx.py --native-tables` converts supported simple tables with text color, shading, `rowspan`, and `colspan` into native Word tables while retaining PNG fallback behavior for unsupported layouts. The eligibility boundary now explicitly rejects browser-positioned `role="img"` drawings and `colgroup` layout tables, including the restriction-digest maps and gel profiles in the local genetics quiz, because they depend on rendered geometry rather than a logical cell grid.

### Developer Tests and Notes

- `pytest tests/`: 434 passed. `source source_me.sh && python3 tests/e2e/e2e_bbq_tasks_quiz.py`: E2E PASS (quiz and exam modes). Smoke: `output_smoke/quiz_tasks3.csv` (chargaff, who_father, mutant_screen) produced 2 questions, 1 skipped (`mutant_screen.py`: FIB only), 1 gel PNG, DOCX with 1 inline shape and no raw tags.
- Focused permanent tests cover the stable color, choice-layout, matching-spacing, and native-table contracts in `tests/test_bbq_html.py`, `tests/test_text_utils.py`, `tests/test_docx_choice_styles.py`, `tests/test_docx_builder_image_choices.py`, `tests/test_docx_table_builder.py`, `tests/test_layout.py`, and `tests/test_yaml_to_exam_docx_matching.py`. One-time acceptance evidence for the local `genetics_tasks1.csv` rebuild is kept separate: 25 numbered rows with 0 skipped tasks, rendered standard DOCX page inspection, and native-backup inspection showing supported Word tables plus PNG fallback; these artifact counts and page renders are not permanent pytest requirements.
- Acceptance run on the unchanged website `genetics_tasks1.csv` (87 resolved tasks, quiz mode): 82 question blocks / 119 numbered rows across 7 topic chapters, 34 statement images + 34 choice images (68 DOCX inline shapes), 82 key lines, no raw HTML in any paragraph. Skipped with expected reasons: `mutant_screen.py` (FIB only), `chi_square_calculated.py` (NUM only), and `palindrome_sequence_match.py`, `write_pedigree_match.py`, `write_pedigree_match_random.py` (matching prompts were table drawings; fixed later the same day by image-capable prompts). The two NUM/FIB-only rows were then removed from the website CSV at the user's request. Follow-up candidate: render table-drawing prompts as images in matching blocks so the pedigree generators print.
...
- Lowered `DEFAULT_MAX_CHARS_2` in `ef_tools/layout.py` from 49 to 42. The previous 49-character budget was too optimistic for the rendered Liberation Sans 10pt 2-column layout, frequently letting two items pretend to fit on one row when they actually overlapped or wrapped. The new 42 pushes borderline cases to a vertical stack via `auto_layout_for_choices`. Per-question `layout: 1` in YAML still works as the manual override; this just makes the auto-layout default more conservative.
- Replaced soft Word line breaks (`<w:br/>`) with hard paragraph breaks (`<w:p>`) throughout the YAML-to-DOCX pipeline. The 2026-05-06 change "turn YAML newlines and legacy `<br>` tags into Word line breaks" intermixed soft and hard breaks within a single question, which made vertical spacing inconsistent and confusing. Now every `\n` (and legacy `<br>` tag) in YAML statement and matching-prompt text becomes a new paragraph styled the same as the parent (Question Heading/Follow, Matching Prompt). New helper `ef_tools.docx_builder.add_rich_text_paragraphs(doc, style_name, text, prefix='')` splits text on the new module-level regex `_PARAGRAPH_BREAK_RE` (`\r?\n` or `<br\s*/?>` case-insensitive) and emits one paragraph per non-empty piece, applying the optional prefix only to the first. `add_rich_text_runs` no longer calls `run.add_break()`; it now collapses any stray `\n` in segments to spaces (paragraph splitting is the caller's responsibility). Statement and matching-prompt rendering in `yaml_to_exam_docx.py` switched to the new helper. `add_choices_paragraph` in `ef_tools/docx_builder.py` was also rewritten: instead of one paragraph with soft breaks between rows, it now emits one paragraph per row (vertical stack: one per choice; multi-column: one paragraph per `items_per_row` group), each styled `Choices 2/3/4/5`. To preserve tight inter-row spacing within a single choices group, rows after the first force `space_before=0` and rows before the last force `space_after=0`, so only the last row's `space_after` provides the gap before the next question. `add_image_choices_tabbed` was unchanged because its single-row tab-stop layout never used soft breaks.
- Renamed `docx_exam_builder.py` to `yaml_to_exam_docx.py` (via `git mv`) so every root script reads as `<input-format>_to_exam_<output-format>.py` and the input/output direction is unambiguous at `ls`. Updated all references in `devel/measure_image_choices.py`, `tests/test_docx_choice_styles.py`, `tests/test_yaml_to_exam_docx_matching.py` (renamed from `test_docx_exam_builder_matching.py`), `README.md`, `docs/USAGE.md`, and `docs/YAML_EXAM_FORMAT.md`.
...
- Added `docs/CODE_ARCHITECTURE.md` mapping every root script and `ef_tools/` module to its role, plus a data-flow diagram showing the Blackboard-HTML -> YAML -> DOCX pipeline (with optional ZipGrade validation/filter), an extension-points section for adding new converters or styles, and a Known-gaps note about the missing `tests_e2e/` harness. Added `docs/FILE_STRUCTURE.md` with the top-level layout, key subtree maps for `ef_tools/`, `styles/`, `tools/`, `tests/`, and `devel/`, the `.gitignore`-derived list of generated artifacts, the docs map, and a "where to add new work" table. Linked both new docs and the new `docs/RELATED_PROJECTS.md` from `README.md`.
- Added `docs/RELATED_PROJECTS.md` linking to https://github.com/vosslab/qti-package-maker (source of the bbq_text input for `bbq_to_exam_yaml.py` and origin of the `MATCH(question_text, prompts_list, choices_list)` schema mirrored in `docs/YAML_EXAM_FORMAT.md`) and https://github.com/vosslab/biology-problems (upstream content authoring repo whose Python style is kept in sync with this repo's).
- Replaced the obsolete `docs/ROADMAP.md` content. The previous file described the long-removed ODT pipeline (Phases 1-4 covering `odt_utils.py`, `extract_odt_styles.py`, etc.), which has been entirely superseded by the DOCX pipeline. The new ROADMAP names the current state, near-term tuning work (image-choice column caps measured via `devel/measure_image_choices.py`), and remaining known doc gaps. Out-of-scope section explicitly bans reintroducing ODT output and the one-shot HTML-to-DOCX path that was deleted on 2026-05-07. Confirmed both `LICENSE.LGPL_v3` (source code) and `LICENSE.CC_BY_4_0` (non-code material) are present at the repo root, so the previously-flagged license gap was incorrect.
...
- Added `devel/measure_image_choices.py`, a repeatable measurement rig for image-choice column sizing. Generates noise PNGs, builds a one-question-per-col-count exam through the production pipeline (`docx_exam_builder.build_document`), converts each page to PNG via LibreOffice + ImageMagick, and prints a per-cols table comparing each rendered image's right edge to its target tab stop so future `IMAGE_CHOICE_MAX_WIDTH_BY_COLS`, `layout_tab_stops`, and `choice_indent` changes can be tuned without ad-hoc scratch scripts. Runs with `source source_me.sh && python3 devel/measure_image_choices.py`; output lands in `/tmp/image_choice_measure/` for inspection.
...
- **WP-1.4: Auto-select question style (Question Heading vs. Question Follow)**
  ...
  - Added comprehensive documentation to `ODT_EXAM_STYLES.md` explaining the auto-selection rules and when each style is used.
...
- Added `odt_exam_builder.py` -- generates fully styled ODT exam documents from YAML input. Embeds all named styles from `ODT_EXAM_STYLES.md` including page layouts, question formatting, multiple choice layouts (Choices3/4/5), tables with colored headers, and image embedding.
...
- Chose `lxml` + `zipfile` over `odfpy` for ODT manipulation per [ROADMAP.md](ROADMAP.md). This avoids a new dependency and gives full control over XML structure.
